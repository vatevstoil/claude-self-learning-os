...
| **L3** | Pinecone 2.0 (semantic search) | "What did we discuss/decide before?" | When recalling history |
...
3. IF user mentions past decision → Pinecone search
...
IF user asks about external topic / new technology:
  ...
  3. After research → optionally vectorize into Pinecone

IF user asks about past conversation / decision:
  1. Pinecone → semantic search in relevant namespace
...
Pinecone search:   ~300 tokens (per query)
...
2. IF significant decisions made → save to Pinecone:
