#!/usr/bin/env python3
"""Auto-save session learnings to long-term memory when a Claude Code session ends.
...
(which routes to the active memory backend: local bge-m3 or Pinecone cloud).
...
def save_to_pinecone(namespace: str, entry_id: str, content: str,
    ...
    try:
        ...
        if result.returncode == 0:
            log.info("Saved to memory: ns='%s' id='%s' type='%s'",
                     namespace, entry_id, metadata['type'])
            return True
        elif result.returncode == 3:
            # Memory backend unavailable — pinecone.py QUEUED it to pending-saves.jsonl
            # (NOT lost). Auto-replays once the backend is available again. Treat as handled.
            log.info("QUEUED (memory backend unavailable — replays after heal): ns='%s' id='%s'",
                     namespace, entry_id)
            return True
        else:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Auto-save session learnings to long-term memory (Claude Code Stop hook)."
    )
    parser.add_argument(
        "--manual",
        action="store_true",
    ...
    # Detect namespace, then canonicalize through ns_util so Stop-hook saves
    # converge on the SAME namespace as pinecone.py CLI saves. Without this the
    # hook re-creates fragmented aliases (e.g. "Claude Trading" instead of
    # "Trading", "Facturka.bg" instead of "Fakturka.bg") and silently undoes
    ...
    if namespace:
        try:
            ...
            canonical = sanitize_ns(namespace)
            if canonical != namespace:
                log.info("Canonicalized namespace '%s' → '%s'", namespace, canonical)
            namespace = canonical
        except Exception as exc:
            log.warning("ns_util canonicalization failed for '%s': %s", namespace, exc)
    if not namespace:
    ...
    if args.manual and not Path(transcript_path).exists():
        log.info("[manual mode] transcript_path='%s' not found — skipping memory save", transcript_path)
        sys.exit(0)
