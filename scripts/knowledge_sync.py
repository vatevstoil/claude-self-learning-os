...
def _run(ns: str, globs: list[str], force: bool, timeout: int = 300) -> str:
...
def main() -> None:
    ...
    base_research = wm.get("base_research_path", r"{{RESEARCH_PATH}}")
    ...
    # Research wikis (live in {{RESEARCH_PATH}}, NOT {{WIKI_PATH}}, and are
    # status="missing-wiki" so the active-loop above skips them). Their deep
    # knowledge lives in wiki/concepts/ + wiki/sources/ — embed it to the
    # project namespace so cross-session recall can actually surface it.
    # Opt-in via metadata "research": true; folder defaults to the entry key,
    # overridable with "research_path".
    for name, info in (meta or {}).items():
        if not (info or {}).get("research"):
            continue
        ns = wm.get("mapping", {}).get(name, name)
        folder = (info or {}).get("research_path", name)
        # Recursive (**) so nested knowledge dirs (e.g. concepts/blueprints/)
        # are covered — a flat concepts/*.md missed 64 N8N blueprint files.
        globs = [
            f"{base_research}/{folder}/wiki/concepts/**/*.md",
            f"{base_research}/{folder}/wiki/sources/**/*.md",
            f"{base_research}/{folder}/wiki/summaries/**/*.md",
            f"{base_research}/{folder}/wiki/entities/**/*.md",
            f"{base_research}/{folder}/wiki/overview.md",
        ]
        # Research wikis can be large on FIRST embed (e.g. 100+ concept files);
        # give them a longer timeout. After the content-hash manifest is
        # populated, subsequent runs are incremental and finish in seconds.
        results.append(_run(ns, globs, force, timeout=900))
