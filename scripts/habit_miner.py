...
_CORRECTION_PHRASES = (
    "не", "не така", "не е", "не прави", "не това", "недей", "невярно",
    "грешно", "греша", "wrong", "стоп", "stop", "no that",
)
# Word-boundary match: bare "не" must NOT fire as a substring of unrelated words
# (it appears inside countless Bulgarian words). \w in a Python str regex is
# Unicode-aware, so the lookarounds correctly respect Cyrillic letter boundaries.
# Before this fix ~99% of Bulgarian user messages were misclassified as
# corrections, starving the habit graduation pipeline (only 14/9634 graduated).
_CORRECTION_RE = re.compile(
    r"(?<!\w)(?:" + "|".join(re.escape(p) for p in _CORRECTION_PHRASES) + r")(?!\w)",
    re.IGNORECASE,
)
...
def _is_correction(text: str) -> bool:
    ...
    known correction phrases as a whole word (case-insensitive).
