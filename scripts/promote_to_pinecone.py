#!/usr/bin/env python3
"""promote_to_pinecone.py - Manually save a confirmed pattern to long-term memory.

Use this for high-value learnings that should NEVER expire.
Sets type=promoted, ttl_days=9999, prefixes ID with 'promoted-'.

Routes to the active MEMORY_BACKEND (local bge-m3 by default; Pinecone cloud
when MEMORY_BACKEND=pinecone). Non-ASCII namespaces (e.g. Cyrillic "Петър Дънов")
are automatically transliterated to an ASCII slug for the namespace + vector ID.
The original namespace is preserved in metadata as `display_namespace` so
retrieval tools can show the human name.
...
def main():
    ...
    # Skipped silently when memory backend is unavailable (e.g. offline run).
    if not args.force:
        # bge-m3 (local) cosine scores run lower than e5 — a near-duplicate tops
        # out ~0.72, not ~0.88. Use a backend-calibrated threshold or the dup
        # guard never fires locally and redundant patterns accumulate.
        import os as _os
        _dup_thresh = 0.72 if _os.environ.get("MEMORY_BACKEND", "local").strip().lower() == "local" else 0.88
        dupes = _check_duplicates(safe_ns, args.text, threshold=_dup_thresh)
        if dupes:
